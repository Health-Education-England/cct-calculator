...
export const CalculationRow: FC<CalculationRowProps> = ({
  ...
  const calculatedDate = dayjs(programmeEndDate).add(cumulativeDaysAdded ?? 0, "day");
  const displayedResultingCctDate =
    cumulativeDaysAdded !== undefined && calculatedDate.isValid()
          ...
          <div className="nhsuk-grid-column-one-quarter">
            <FormField label="New CCT Date" id="">
              <div className="nhsuk-input">
                {displayedResultingCctDate
                  ? dayjs(displayedResultingCctDate).format("DD/MM/YYYY")
                  : "Not calculated"}
              </div>
            </FormField>
          </div>

...
## GitHub Package Setup (Required)

This project depends on `@health-education-england/cct-calc-core` from GitHub Packages.

Before running `npm install`, create a **GitHub Personal Access Token (classic)** with the `read:packages` scope.
...
- The project `.npmrc` already points the `@health-education-england` scope to `https://npm.pkg.github.com`.
- You only need the token for package install/update commands (`npm install` / `npm ci`).
- You do not need the `repo` PAT scope for this package; `read:packages` is sufficient.
